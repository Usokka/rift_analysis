...
type Metadata = {
  ...
  teams: { team_id: string; team_name: string; league: string; year: number; matches: number }[];
};
...
function formatValue(item?: Metric, compact = false) {
  ...
  if (item.unit === 'or') return `${item.value >= 0 ? '+' : ''}${Math.round(item.value).toLocaleString('fr-FR')}`;
  ...
  return (
    <article className={`metric-block${prominent ? ' prominent' : ''}`}>
      ...
      <div className="metric-meta">
        ...
        <span>{item.sample_size}/{item.eligible_sample_size} matchs · {coverage}% couverts</span>
      </div>
    </article>
  );
}
...
function TrendChart({ points }: { points: Trend[] }) {
  const values = points.map((point) => point.gold_diff_at_15).filter((value): value is number => value !== null);
  ...
  return (
    <div className="chart" aria-label="Évolution hebdomadaire de la différence d’or à 15 minutes">
      ...
      <div className="chart-scale"><span>{Math.round(max).toLocaleString('fr-FR')}</span><span>0</span><span>{Math.round(min).toLocaleString('fr-FR')}</span></div>
      <div className="chart-dates"><span>{new Date(points[0].week).toLocaleDateString('fr-FR')}</span><span>{new Date(points.at(-1)!.week).toLocaleDateString('fr-FR')}</span></div>
    </div>
  );
}
...
export function App() {
  ...
  useEffect(() => {
    ...
    fetch(analyticsUrl('/api/v1/analytics/metadata', 'metadata.json'), { signal: controller.signal })
      ...
      .then((body) => {
        ...
        setLeagueKey(`${first.year}|${first.league}`);
      })
      .catch((error: unknown) => {
  ...
  useEffect(() => {
    ...
    if (!teams.some((team) => team.team_id === teamId)) setTeamId(teams[0].team_id);
    setSplit('');
  }, [leagueKey, teams, teamId]);
  ...
  useEffect(() => {
    ...
    fetch(analyticsUrl(`/api/v1/analytics/overview?${params}`, 'overview.json'), { signal: controller.signal })
      ...
      .then((body) => {
        setOverview(body);
        setState(body ? 'ready' : 'empty');
      })
      .catch((error: unknown) => {
        if (!(error instanceof DOMException && error.name === 'AbortError')) setState('error');
      });
    return () => controller.abort();
  }, [league, year, teamId, split, startDate, endDate, attempt]);

  const keyMetrics = overview ? ['T01', 'T05', 'T08'].map((id) => metric(overview.team_metrics, id)) : [];
    ...
    <div className="workspace">
      ...
      <main id="main">
        <header className="topbar">
          <div><p className="eyebrow">LEAGUE OF LEGENDS · ESPORTS</p><h1>{overview?.filters.team_name ?? 'Performance Review'}</h1></div>
